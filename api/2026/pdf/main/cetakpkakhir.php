...
from tb_data_ttd_akhir where kode_satker_pejabat = '" . $kode_satker . "' and level_pejabat='Satker'";
//echo $sqlsatkerttd;
//die;
...
from tb_data_ttd_akhir where kode_satker_pejabat = '" . $kdbalai . "' and level_pejabat='Balai'";
//echo $sqlbalaittd;
//die;
...
if (((float) $levelpiu > 7) && ((float) $levelpiu <> 31) && ((float) $levelpiu <> 41) && ((float) $levelpiu <> 9)) {
	$sqlbalaittd = "select nama_pejabat, jabatan_pejabat, filenya_ttd
...
if (((float) $levelpiu == 6)) {
	$sqlbalaittd = "select nama_pejabat, jabatan_pejabat, filenya_ttd
	from tb_data_ttd_akhir where kode_satker_pejabat='03694117'";
	//echo $sqlbalaittd;
	//die;
	$resultbalaittd = mysqli_query($link, $sqlbalaittd);
	while ($rowbalaittd = mysqli_fetch_assoc($resultbalaittd)) {
...
if (((float) $levelpiu == 31)) {
	$sqlbalaittd = "select nama_pejabat, jabatan_pejabat, filenya_ttd
	from tb_data_ttd_akhir where level_pejabat='DirSupan'";
	//echo $sqlbalaittd;
	//die;
	$resultbalaittd = mysqli_query($link, $sqlbalaittd);
	while ($rowbalaittd = mysqli_fetch_assoc($resultbalaittd)) {
...
if (((float) $levelpiu == 9)) {
	$sqlbalaittd = "select nama_pejabat, jabatan_pejabat, filenya_ttd
	from tb_data_ttd_akhir where level_pejabat='DirSSPSDA'";
	//echo $sqlbalaittd;
	//die;
	$resultbalaittd = mysqli_query($link, $sqlbalaittd);
	while ($rowbalaittd = mysqli_fetch_assoc($resultbalaittd)) {
			...
			while ($rowlevelISPSK = mysqli_fetch_assoc($resultlevelISPSK)) {
				...
				while ($rowlevelKEGIATAN = mysqli_fetch_assoc($resultlevelKEGIATAN)) {
					...
					while ($rowlevelSASARANPROGRAM = mysqli_fetch_assoc($resultlevelSASARANPROGRAM)) {
						...
						if (!$found) {
							$currentY = $pdf->GetY();
							if ($rowlevelSASARANPROGRAM['textindikator'] == 'DUKUNGAN MANAJEMEN' && (float) $currentY > 90) {
								$pdf->AddPage('L', $resolution);
								$pdf->SetFont('dejavusans', '', 9);
								$html = '
								<table border="0"  cellpadding="2" style="width:100%;" >
...
<tr>
<td align="center" width="1000px">	
<b>BERITA ACARA KESEPAKATAN CAPAIAN KINERJA TAHUN ' . $tahunsekarang . '</b>
</td>
</tr>';
if (strlen($nama_satker . $nama_balai) < 100) {
	$html = $html . '
	<tr>
	<td align="center" width="1000px">	
	<b>' . $nama_satker . ' - ' . $nama_balai . '</b>
	</td>
	</tr>';
} else {
	$html = $html . '<tr>
...
$html = $html . '
<tr>
<td align="center" width="500px" rowspan="2" style="background-color: #D3D3D3;">
<b><br>SASARAN PROGRAM / SASARAN KEGIATAN / INDIKATOR SASARAN PROGRAM</b>
</td>
<td align="center" width="200px" style="background-color: #D3D3D3;">
<b>TARGET</b>
</td>
...
if ($num > 0) {
	if ($result) {
		while ($row = mysqli_fetch_assoc($result)) {
			...
			$kinerja = 0;
			if ($row['hitungan_pk'] == '1') {
				if ($target_tahun_berjalan == 0) {
					if ($target_tahun_berjalan == 0 && $capaian_outcome == 0) {
						$kinerja = 100;
					} else {
						$kinerja = 0;
					}
					...
					if ($target_tahun_berjalan == 0 && $capaian_outcome == 0) {
						...
						$kinerja = ((float) $capaian_outcome / (float) $target_tahun_berjalan) * 100;
					}
				}
			} else {
				if ($output_tahun_berjalan == 0) {
					if ($output_tahun_berjalan == 0 && $capaian == 0) {
						$kinerja = 100;
					} else {
						$kinerja = 0;
					}
					...
					if ($output_tahun_berjalan == 0 && $capaian == 0) {
						...
						$kinerja = ((float) $capaian / (float) $output_tahun_berjalan) * 100;
					}
				}
			}
...
$html = $html . '
<tr>
<td align="center" width="240px" rowspan="2" style="background-color: #D3D3D3;">
<b>SASARAN PROGRAM / SASARAN KEGIATAN / INDIKATOR SASARAN PROGRAM</b>
</td>
<td align="center" width="160px" style="background-color: #D3D3D3;">
...
if ($num > 0) {
	if ($result) {
		while ($row = mysqli_fetch_assoc($result)) {
			...
			if ($output_tahun_berjalan == 0) {
				if ($row['hitungan_pk'] == '1') {
					if ((float) $output_tahun_berjalan == 0 && (float) $capaian == 0) {
						$kinerja = 100;
					} else {
			...
			$kinerja_outcome = 0;
			if ((float) $target_tahun_berjalan == 0) {
				if ($row['hitungan_pk'] == '1') {
					if ((float) $target_tahun_berjalan == 0 && (float) $capaian_outcome == 0) {
						$kinerja_outcome = 100;
					} else {
						if ((float) $target_tahun_berjalan == 0) {
							$kinerja_outcome = 0;
						} else {
							$kinerja_outcome = ((float) $capaian_outcome / (float) $target_tahun_berjalan) * 100;
						}
					}
				} else {
					if ((float) $target_tahun_berjalan == 0 && (float) $capaian == 0) {
						$kinerja_outcome = 100;
					} else {
						if ((float) $target_tahun_berjalan == 0) {
							$kinerja_outcome = 0;
						} else {
							$kinerja_outcome = ((float) $capaian / (float) $target_tahun_berjalan) * 100;
						}
					}
				}
			} else {
			...
			if ($row['hitungan_pk'] == '1') {
				...
				$output_tahun_berjalan = number_format($output_tahun_berjalan, $belakangkoma, ",", ".");
				$target_tahun_berjalan = number_format($target_tahun_berjalan, $belakangkoma, ",", ".");
				$capaian_outcome = number_format($capaian, $belakangkoma, ",", ".");
				$capaian = number_format($capaian, $belakangkoma, ",", ".");
				$kinerja = number_format($kinerja, 2, ",", ".");
				$kinerja_outcome = number_format($kinerja_outcome, 2, ",", ".");
			}
