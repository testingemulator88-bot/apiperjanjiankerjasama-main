...
if ($numcek > 0) {
    if (($Bearer <> '') && ($tempBearer[1] <> '')) {
        ...
        if ($kdunor <> '') {
            $filter_query = $filter_query . " and a.kdunor='" . $kdunor . "'";
        }
        $kategorisatker = fixup($_GET['kategorisatker']);
        if ($kategorisatker <> '') {
            $filter_query = $filter_query . " and a.kdbalai='" . $kategorisatker . "'";
        }
        $cantekbalai = fixup($_GET['cantekbalai']);
        if ($cantekbalai == '34') {
            $filter_query = $filter_query . " and a.kdbalai in (select id from master_kategori_satker where deleted='0' and level_piu='32')";
        }
        if ($cantekbalai == '21') {
            $filter_query = $filter_query . " and a.level_piu='6'";
        }
        $myArray = array();
        ...
        a.level_piu=e.id
        where a.deleted='0' and a.kode_satker<>'03'
        " . $filter_query . "
