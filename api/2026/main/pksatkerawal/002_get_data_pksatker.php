...
if ($numcek > 0) {
    if (($Bearer <> '') && ($tempBearer[1] <> '')) {
        ...
        $kodekegiatan = "";
        $sqlindikator = "  select kode,kode_unique from tb_indikator_awal where id = 
        (select id_parent from tb_indikator_awal where id = (select id_parent from tb_indikator_awal where id = '" . $id . "'))";
        ...
        $sqlindikatorkode_unique = "  select kode,kode_unique,belakangkoma from tb_indikator_awal where id = '" . $id . "'";
        $resultindikatorkode_unique = mysqli_query($link, $sqlindikatorkode_unique);
        while ($rowindikatorkode_unique = mysqli_fetch_assoc($resultindikatorkode_unique)) {
            $kode_unique = $rowindikatorkode_unique['kode_unique'];
            $belakangkoma = $rowindikatorkode_unique['belakangkoma'];
        }
        ...
        for ($x = $tahun_awalcek; $x <= $tahun_akhircek; $x++) {
            ...
            from tb_data_pk_akhir a where a.deleted='0' and a.tahun='" . $x . "' " . $filterquery . "";
            $resultdatacek = mysqli_query($link, $sqldatacek);
            $numdatacek = mysqli_num_rows($resultdatacek);
            if ($numdatacek == 0) {
                $sqlinsert = "insert into tb_data_pk_akhir (id,id_indikator,kode_tahun,tahun,id_satker,kode_satker
            ...
            $sqldatacek = "select a.id
            from tb_data_pk_awal a where a.deleted='0' and a.tahun='" . $x . "' " . $filterquery . "";
            $resultdatacek = mysqli_query($link, $sqldatacek);
            $numdatacek = mysqli_num_rows($resultdatacek);
            if ($numdatacek == 0) {
                $sqlinsert = "insert into tb_data_pk_awal (id,id_indikator,kode_tahun,tahun,id_satker,kode_satker
                ,proyek,nilai,rumus,deleted) values 
                (null,'" . $id . "','" . $tahun . "','" . $x . "','" . $id_satker . "','" . $kode_satker . "'
                ,'" . $textindikator . "','" . $isinilaifinal . "','" . $rumus . "','0')";
                $resultinsert = mysqli_query($link, $sqlinsert);
            ...
            if ($result) {
                while ($row = mysqli_fetch_assoc($result)) {
                    ...
                    for ($x = $tahun_awal; $x <= $tahun_akhir; $x++) {
                        ...
                        if ($x < $tahunsekarang) {
                            if ($hitungan_pk <> '1') {
                                $sqldata = "select a.id,a.kode_satker,a.proyek,a.volume,a.capaian as target,a.nilai,a.rumus
                                ,a.skor_komponen_a,a.skor_komponen_b,a.skor_komponen_c
                                from tb_data_pk_akhir a where a.deleted='0' and a.tahun='" . $x . "' " . $filterquery . "";
                            } else {
                                $sqldata = "select a.id,a.kode_satker,a.proyek,a.volume,a.capaian_outcome as target,a.nilai,a.rumus
                                ,a.skor_komponen_a,a.skor_komponen_b,a.skor_komponen_c
                                from tb_data_pk_akhir a where a.deleted='0' and a.tahun='" . $x . "' " . $filterquery . "";
                            }
                        } else {
                        ...
                        if ($numdata > 0) {
                            ...
                            if ($resultdata) {
                                while ($rowdata = mysqli_fetch_assoc($resultdata)) {
                                    if ($kode_unique == 'IKSK7693SK101') {
                                        $baseline = $baseline + $rowdata['volume'];
                                    } else {
                                    ...
                                    $volume = (float) $rowdata['volume'];
                                    $target = (float) $rowdata['target'];

                                    $myArrayemon = array();
                                    if ($x < $tahunsekarang) {
                                        $sqlemon = "select a.id,a.tahun,a.kode_satker,a.kode_emon,a.nilai, a.rumus, a.nmpaket as nmpaketdata
                                        ,x.kode,x.nmpaket,x.pgrupiah,x.vol,x.sat,a.latitude,a.longitude,a. capaian_outcome as target,a.capaian as outcome
                                        ,a.skor_komponen_a,a.skor_komponen_b,a.skor_komponen_c
                                        from tb_data_pk_emon_akhir a 
                                        left join paket_pk_akhir x
                                        on a.kode_emon = x.kode
                                        and YEAR(x.tanggaldata)='" . $x . "'
                                        and a.kode_emon in (select z.kode from paket_pk_akhir z where YEAR(z.tanggaldata)='" . $x . "'
                                        ...
                                        on a.kode_emon = x.kode
                                        and YEAR(x.tanggaldata)='" . $x . "'
                                        and a.kode_emon in (select z.kode from paket_pk_awal z where YEAR(z.tanggaldata)='" . $x . "'
                                        and (z.kdsatker = '" . $kode_satkeremon . "' or z.kdsatker = '" . $kode_satkeremon_old . "'))
                                        where a.deleted='0' and a.tahun='" . $x . "' " . $filterquery . "";
                                        // $sqlemon;
                                    }
                                    ...
                                    while ($rowemon = mysqli_fetch_assoc($resultemon)) {
                                        ...
                                        if ($hitungan_pk <> '1') {
                                            for ($z = 0; $z < count($tempnilai); $z++) {
                                                ...
                                                $myArraynilaiemon[] = (object)[
                                                    'nilai' => round($nilai, $belakangkoma),
                                                    'rumus' => $temprumus[$z],
                                                    'disabled' => $disabled,
                                                ];
                                            }
                                        ...
                                        $outcome_emon = $rowemon['outcome'];
                                        $target_emon = $rowemon['target'];
                                        //die;
                                        $myArrayemon[] = (object)
                                        [
                                            'id' => (float) $rowemon['id'],
                                            ...
                                            'outcome' => replace_dot_koma_tiga($outcome_emon),
                                            'target' => replace_dot_koma_tiga($target_emon),
                                    ...
                                    $isitambahbaseline = 0;
                                    if ($kumulatif == '0') {
                                        $isitambahbaseline = $target;
                                    } else {
                                        $isitambahbaseline = $baseline;
                                    }
                                    ...
                                    [
                                        ...
                                        'tambahbaseline' => replace_dot_koma_tiga($isitambahbaseline),
                                ...
                                for ($z = 0; $z < count($tempnilaiproyek); $z++) {
                                    if ($hitungan_pk <> '1') {
                                        for ($z = 0; $z < count($tempnilaiproyek); $z++) {
                                            ...
                                            $myArraynilaiproyek[] = (object)[
                                                'data_ke' => (float) $y,
                                                'nilai' => round((float) $nilai, $belakangkoma),
                                            ];
                                        }
                                    }
                                }
