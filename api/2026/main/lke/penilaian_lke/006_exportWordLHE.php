...
header('Content-Disposition: attachment; filename="LHE_Satker_Filled.docx"');
...
if ($numcek > 0) {
    ...
    if (($Bearer <> '') && ($tempBearer[1] <> '')) {
        ...
        if(strstr($level, '1') || strstr($level, '5')){
            ...
            if(strstr($level, '1')){
                $sql_cek_user   = "SELECT b.kode_satker FROM master_satker as a 
                                    ...
                                    WHERE b.level in (7) and b.kode_satker = '". $kode_satker . "' GROUP BY b.kode_satker";
            }
            else{
                $sql_cek_user   = "SELECT b.kode_satker FROM master_satker as a 
                                    INNER JOIN tb_user as b ON b.kode_satker = a.kode_satker and b.deleted='0'
        ...
        if($id_user != '' && $tahun != ''){
            $sql_satker     = "SELECT kode_satker, kdbalai, replace(nama_satker, 'SATKER', '') as nama_satker FROM master_satker WHERE kode_satker = '". $id_user ."' AND deleted = '0'";
            ...
            $templatePath =  __DIR__ . '/Format_LHE_Satker.docx';
            ...
            if($ttd_num > 0){
                ...
                $no_surat_ttd   = $row_ttd['no_surat'];
                $tempat_ttd     = $row_ttd['tempat_ttd'];
                $tgl_ttd        = tanggal_indodoank($row_ttd['tgl_surat']);
                $nip_pejabat    = $row_ttd['nip_pejabat'];
                $nama_pejabat   = $row_ttd['nama_pejabat'];
                $jabatan        = $row_ttd['jabatan'];
            }
            else{
                $no_surat_ttd   = '';
                $tempat_ttd     = '';
                ...
                $nip_pejabat    = '';
                $nama_pejabat   = '';
                $jabatan        = 'Kepala ' . ucwords(strtolower($kategori_satker));
            }
            
            // set Value Kop Surat LHE
            $template->setValue('no_surat', $no_surat_ttd);
            $template->setValue('tempat', $tempat_ttd);
            $template->setValue('tgl_ttd', $tgl_ttd);
            $template->setValue('tahun', (float)$tahun);
            $template->setValue('satker', 'Satker '. ucwords(strtolower($nama_satker)));
                            ...
                            if($subKomponenResult){
                                while($rowSubKomponen = mysqli_fetch_assoc($subKomponenResult)){
                                    ...
                                    $sql_kriteria_penilaian1 = "SELECT sum(a.kriteria_penilaian1) as pemenuhan_kriteria, sum(a.kriteria_penilaian2) as total_tkmn, 
                                                                ...
                                                                WHERE a.tahun = '". $tahun ."' AND a.kode_satker = '". $id_user ."' AND a.level in (7) AND b.id_subkomponen = '". $rowSubKomponen['id_subkomponen'] ."' 
                                        ...
                                        if($kriteriaResult){
                                            while($rowKriteria = mysqli_fetch_assoc($kriteriaResult)){
                                                $cek_lke        = "SELECT a.* FROM hasil_lke as a
                                                                    WHERE a.id_kriteria = '". $rowKriteria['id_kriteria'] ."' AND a.tahun = '". $tahun ."' AND a.kode_satker ='". $id_user ."' AND a.is_active = '1'
                                                                    AND a.level in (7) group by a.id_hasil_lke
                                                                    ";
            ...
            // tabel 1 di word
            // Clone row Komponen1 di template
            $template->cloneRow('data_komponen0', count($komponenArray));
            
            foreach($komponenArray as $key => $value){
                $subkomponen_var = '';
                foreach($value->detail_subkomponen as $key2 => $value2){
                    if(!empty($value2->nama_subkomponen) && !is_null($value2->nama_subkomponen)){
                        $subkomponen_var .= chr(($key2 + 1) + 64) .'. ' . $value2->nama_subkomponen . "\n";
                    }
                }

                $template->setValue('no0#' . ($key+1), (float)$value->kode_komponen);
                $template->setValue('data_komponen0#' . ($key+1), ucwords(strtolower(strip_tags($value->nama_komponen))));
                $template->setValue('subkomponen0#' . ($key+1), $subkomponen_var);
            }
            ...
            if ($KomponenNum2 > 0) {
                if ($KomponenResult2) {
                    while ($rowKomponen2 = mysqli_fetch_assoc($KomponenResult2)) {
                        ...
                        $subKomponenSql2    = "SELECT d.deskripsi_rekomendasi from subkomponen as b 
                                                ...
                                                and b.id_komponen = '". $rowKomponen2['id_komponen'] ."' AND d.level in (7) GROUP BY d.id_hasil_lke";
                        ...
                        while($rowSubKomponen2 = mysqli_fetch_assoc($subKomponenResult2)){
                            $subKomponenArray2[] = (object)[
                                'rekomendasi' => html_entity_decode($rowSubKomponen2['deskripsi_rekomendasi']),
                            ];
                        }
                        ...
                        [
                            ...
                            'detail_rekomendasi'    => $subKomponenArray2
                        ];
                    }
                }
            }
            ...
            foreach($komponenArray2 as $key => $value){
                $rekomendasi_var = '';
                foreach($value->detail_rekomendasi as $key2 => $value2){
            ...
            $template->setValue('ttd_lhe', $jabatan);
            $template->setValue('nama_pejabat', $nama_pejabat);
            $template->setValue('nip', $nip_pejabat);
            
            header("Content-Description: File Transfer");
            header('Content-Disposition: attachment; filename="LHE_Satker_'. $nama_satker .'_'. date('YmdHis') .'.docx"');
            header('Content-Type: application/vnd.openxmlformats-officedocument.wordprocessingml.document');
            header('Cache-Control: must-revalidate');
            header('Pragma: public');
            header('Expires: 0');
