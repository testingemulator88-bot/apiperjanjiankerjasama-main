<?php
    ...
    if (($Bearer <> '') && ($tempBearer[1] <> '')) {
        if(strstr($level, '1') || strstr($level, '5')){
            ...
            if(strstr($level, '1')){
                // $sql_cek_user   = "SELECT id FROM tb_user WHERE kode_satker = '". $kode_satker ."' AND level = '7' AND id in (SELECT id_user FROM hasil_lke WHERE tahun = '". $tahun ."' AND kode_satker = '". $kode_satker ."') AND deleted = '0'";
        ...
        if($id_user != '' && $tahun != ''){
            $sql_satker     = "SELECT kode_satker, kdbalai, nama_satker FROM master_satker WHERE kode_satker = '". $kode_satker ."' AND deleted = '0'";
            ...
            foreach($komponenArray as $i => $row){
                ...
                foreach($row->detail_subkomponen as $j => $subRow){
                    ...
                    foreach($subRow->detailKriteria as $k => $kriteriaRow){
                        ...
                        // $sheet->mergeCells('I' . $rowNum . ':I' . ($rowNum + $subRow->rowspan - 1));
                        // $sheet->setCellValue('I' . $rowNum, (float)$subRow->pemenuhan_kriteria);
                        ...
                        // $sheet->mergeCells('M' . $rowNum . ':M' . ($rowNum + $subRow->rowspan - 1));
                        // $sheet->setCellValue('M' . $rowNum, $subRow->nilai_evaluasi);

                        // $sheet->mergeCells('N' . $rowNum . ':N' . ($rowNum + $subRow->rowspan - 1));
                        // $sheet->setCellValue('N' . $rowNum, (float)$subRow->nilai);
                        ...
                        // $sheet->getStyle('I' . $rowNum)->getNumberFormat()->setFormatCode(\PhpOffice\PhpSpreadsheet\Style\NumberFormat::FORMAT_NUMBER_COMMA_SEPARATED1);
                        // $sheet->getStyle('I' . $rowNum)->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER);
                        // $sheet->getStyle('I' . $rowNum)->getAlignment()->setVertical(\PhpOffice\PhpSpreadsheet\Style\Alignment::VERTICAL_TOP);
                        ...
                        // $sheet->getStyle('M' . $rowNum)->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER);
                        // $sheet->getStyle('M' . $rowNum)->getAlignment()->setVertical(\PhpOffice\PhpSpreadsheet\Style\Alignment::VERTICAL_TOP);

                        // $sheet->getStyle('N' . $rowNum)->getNumberFormat()->setFormatCode(\PhpOffice\PhpSpreadsheet\Style\NumberFormat::FORMAT_NUMBER_COMMA_SEPARATED1);
                        // $sheet->getStyle('N' . $rowNum)->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER);
                        // $sheet->getStyle('N' . $rowNum)->getAlignment()->setVertical(\PhpOffice\PhpSpreadsheet\Style\Alignment::VERTICAL_TOP);
            ...
            $rekapSheet->getStyle('A5:D' . ($rowRekap-1))->getBorders()->getAllBorders()->setBorderStyle(Border::BORDER_THIN);
