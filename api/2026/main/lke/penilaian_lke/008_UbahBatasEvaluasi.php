...
if ($numcek > 0) {
    ...
    if (($Bearer <> '') && ($tempBearer[1] <> '') && ($id_kriteria <> '') && ($tahun <> '') && ($kode_satker <> '')) {
        if($tanggal_evaluasi != '' && !is_null($tanggal_evaluasi)){
            $sql = "UPDATE hasil_lke SET tanggal_evaluasi = '" . $tanggal_evaluasi . "', tanggapan_evaluator = '" . $tanggapan_evaluator . "', 
                    deskripsi_rekomendasi = '" . $rekomendasi_evaluator . "', status_tindak_lanjut = '" . $tindak_lanjut . "', status_lke = '4' 
                    WHERE id_hasil_lke = '" . $id_lke . "'";
            
        }
        else{
            $sql = "UPDATE hasil_lke SET tanggapan_evaluator = '" . $tanggapan_evaluator . "', 
                    deskripsi_rekomendasi = '" . $rekomendasi_evaluator . "', status_tindak_lanjut = '" . $tindak_lanjut . "', status_lke = '4' 
                    WHERE id_hasil_lke = '" . $id_lke . "'";
        }
        // echo $sql;
