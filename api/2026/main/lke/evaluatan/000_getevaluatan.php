...
if ($numcek > 0) {
    ...
    if (($Bearer <> '') && ($tempBearer[1] <> '')) {
        ...
        $sqljumlah = "SELECT count(a.id_evaluatan) as jumlahdata FROM evaluatan a 
                        WHERE a.kode_satker = '". $id_user ."' and a.is_active='1' and a.level in (". $level .")  " . $filterquery . " ORDER BY a.id_evaluatan";
        $numjumlah      = 0;
        ...
        $myArray = array();
        $sql    = "SELECT a.id_evaluatan, a.nip, a.tim, c.nama_kategori as wilayah, a.jabatan, a.tahun
                    FROM evaluatan as a
                    INNER JOIN master_kategori_satker as c ON c.id = a.wilayah
                    WHERE a.kode_satker='". $id_user ."' and a.is_active='1' and a.level in (". $level .")  ". $filterquery ." 
                    GROUP BY a.id_evaluatan, a.nip, a.tim, c.nama_kategori, a.jabatan, a.tahun ORDER BY a.id_evaluatan, a.tim 
                    LIMIT ". $mulai . " , 20 ";
        // echo $sql;
        // die();
