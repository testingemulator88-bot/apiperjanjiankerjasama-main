...
if ($numcek > 0) {
    ...
    if (($Bearer <> '') && ($tempBearer[1] <> '') && ($id <> '')) {
        if ($isi == '0') {
            $sql = " insert into tb_data_pk_tidak_cetak (id, id_indikator, kode_tahun, id_satker, kode_satker, createddate
            , createdby, deleted) VALUES (null, '" . $id . "', '" . $kode_tahun . "', '" . $id_satker . "', '" . $kode_satker . "'
            , SYSDATE(), '" . $updateby . "', '0')";
            //echo $sql;
            //die;
        } else {
            $sqlcekeksist = "select id from tb_data_pk_tidak_cetak where deleted='0'
            ...
            $sql = "update tb_data_pk_tidak_cetak set deleted='1' where id = '" . $idnya . "'";
        }
