...
if ($numcek > 0) {
    ...
    if (($Bearer <> '') && ($tempBearer[1] <> '') && ($id_indikator <> '') && ($id_balai <> '') && ($kode_tahun <> '')) {
        ...
        $sqlindikator = "select * from tb_indikator_awal where deleted='0' and id='" . $id_indikator . "' ";
        //echo $sqlindikator;
        //die;
        ...
        $sqlcek = "select id from tb_data_baseline_pk_balai_awal where deleted='0' and kode_tahun='" . $kode_tahun . "'
        ...
        if ($numcek == 0) {
            $sql = "insert into tb_data_baseline_pk_balai_awal (id,kode_tahun,id_indikator,id_balai,kode_balai,kolom1,kolom2,kolom3,kolom4
            ...
            $sql = "update tb_data_baseline_pk_balai_awal set kolom1='" . $baselinekolom1 . "',kolom2='" . $baselinekolom2 . "'
        ...
        for ($x = $tahun_awal; $x <= $tahun_akhir; $x++) {
            ...
            if ($hitungan_pk_awal == '2' || $hitungan_pk_awal == '3') {
                ...
                $sqlupdatedata = "update tb_data_pk_balai_awal set nilai = '" . $nilaifinal . "', rumus = '" . $masterrumus . "'
                ...
                while ($rowindikator = mysqli_fetch_assoc($resultindikator)) {
                    ...
                    if ($rumuskolom5 <> '') {
                        $targetkumulatif = $tempkolom5[$adi];
                    } else {
                        if ($rumuskolom4 <> '') {
                            $targetkumulatif = $tempkolom4[$adi];
                        } else {
                            if ($rumuskolom3 <> '') {
                                $targetkumulatif = $tempkolom3[$adi];
                            } else {
                                if ($rumuskolom2 <> '') {
                                    $targetkumulatif = $tempkolom2[$adi];
                                } else {
                                    $targetkumulatif = $tempkolom1[$adi];
                                }
                            }
                        }
                    }
                }
                $sqlupdatedata = "update tb_data_pk_balai_awal set 
                isian_kolom = '" . $tempisian_kolom[$adi] . "' ,nilai = '" . $targetkumulatif . "'
                where id_indikator = '" . $id_indikator . "' and tahun = '" . $x . "' and id_balai = '" . $id_balai . "'";
                //echo $sqlupdatedata."<br>";
                $result = mysqli_query($link, $sqlupdatedata);
            }
            $adi++;
        }
