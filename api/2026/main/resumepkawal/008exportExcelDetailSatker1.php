...
if (($numcek > 0) && ($Bearer <> '') && ($tempBearer[1] <> '')) {
        ...
        if ($result) {
            ...
            while ($row = mysqli_fetch_assoc($result)) {
                ...
                $sqlceksatker = "select a.kode_satker,a.nama_satker 
                    , ifnull((select x.nilai from tb_data_pk_awal x where x.deleted='0' and 
                    x.id_indikator='" . $id_indikator . "' and x.tahun=(select tahun
                    from master_pk where id='1') and x.kode_satker=a.kode_satker),0) as isiannilai
                    , ifnull((select x.volume from tb_data_pk_awal x where x.deleted='0' and 
                    x.id_indikator='" . $id_indikator . "' and x.tahun=(select tahun
                    from master_pk where id='1') and x.kode_satker=a.kode_satker),0) as volume
                    , ifnull((select x.target from tb_data_pk_awal x where x.deleted='0' and 
                    x.id_indikator='" . $id_indikator . "' and x.tahun=(select tahun
                    from master_pk where id='1') and x.kode_satker=a.kode_satker),0) as target
                    , ifnull((select x.capaian from tb_data_pk_awal x where x.deleted='0' and 
                    x.id_indikator='" . $id_indikator . "' and x.tahun=(select tahun
                    from master_pk where id='1') and x.kode_satker=a.kode_satker),0) as capaian
                    , ifnull((select x.capaian_outcome from tb_data_pk_awal x where x.deleted='0' and 
                    x.id_indikator='" . $id_indikator . "' and x.tahun=(select tahun
                    from master_pk where id='1') and x.kode_satker=a.kode_satker),0) as capaian_outcome
                    , ifnull((select SUBSTRING_INDEX(nilai, '|', -1) from tb_data_pk_awal x where x.deleted='0' and 
                    x.id_indikator='" . $id_indikator . "' and x.tahun=(select tahun
                    from master_pk where id='1') and x.kode_satker=a.kode_satker),0) as nilai
                    , (select count(zz.id) from tb_data_pk_tidak_cetak_awal zz where zz.id_indikator='" . $id_indikator . "'
                    ...
                    from tb_data_pk_awal b where b.deleted='0' " . $filterquerysatker . "
                ...
                $sqlemon = "select a.id,a.tahun,a.kode_satker,a.kode_emon,a.nilai, a.rumus
                    ...
                    from master_pk where id='1')
                    and a.kode_emon in (select z.kode from paket_pk_awal z where YEAR(z.tanggaldata)=(select tahun
                    from master_pk where id='1')
    ...
    foreach ($myArray as $obj) {
        ...
        $sheet->setCellValue('E' . $baris, replace_comma($obj->volume));
        ...
        $sheet->getStyle('E' . $baris)->getFont()->setBold(true);

        $sheet->setCellValue('F' . $baris, replace_comma($obj->target));
        ...
        $sheet->getStyle('F' . $baris)->getFont()->setBold(true);

        $sheet->setCellValue('G' . $baris, replace_comma($obj->capaian));
        ...
        $sheet->setCellValue('H' . $baris, replace_comma($obj->capaian_outcome));
        ...
        $sheet->getStyle('H' . $baris)->getFont()->setBold(true);
        $baris++;
