...
if (($numcek > 0) && ($Bearer <> '') && ($tempBearer[1] <> '')) {
                                ...
                                while ($rowlevelISPSK = mysqli_fetch_assoc($resultlevelISPSK)) {
                                    ...
                                    while ($rowlevelIKSK = mysqli_fetch_assoc($resultlevelIKSK)) {
                                        ...
                                        $sqlcekbalai = "select a.kdbalai,c.nama_kategori, count(a.kode_satker) as jmlsatker
                                            , (select count(zz.id) from tb_data_pk_tidak_cetak_awal zz where zz.id_indikator='" . $rowlevelIKSK['id'] . "'
                                            and zz.deleted='0' and zz.kode_satker 
                                            in (select b.kode_satker
                                            from tb_data_pk_awal b where b.deleted='0' 
                                            and b.id_indikator='" . $rowlevelIKSK['id'] . "' and b.tahun=(select tahun
                                            from master_pk where id='1'))) as terpilih
                                            ...
                                            from tb_data_pk_awal b where b.deleted='0' " . $filterquerybalai . " " . $filterquerysatker . "
                                            and b.id_indikator='" . $rowlevelIKSK['id'] . "' and b.tahun=(select tahun
                                        ...
                                        while ($rowcekbalai = mysqli_fetch_assoc($resultcekbalai)) {
                                            ...
                                            $sqlceksatker = "select a.kode_satker,a.nama_satker 
                                                , ifnull((select x.volume from tb_data_pk_awal x where x.deleted='0' and 
                                                x.id_indikator='" . $rowlevelIKSK['id'] . "' and x.tahun=(select tahun
                                                from master_pk where id='1') and x.kode_satker=a.kode_satker limit 1),0) as volume
                                                , ifnull((select x.target from tb_data_pk_awal x where x.deleted='0' and 
                                                x.id_indikator='" . $rowlevelIKSK['id'] . "' and x.tahun=(select tahun
                                                from master_pk where id='1') and x.kode_satker=a.kode_satker limit 1),0) as target
                                                , ifnull((select x.capaian from tb_data_pk_awal x where x.deleted='0' and 
                                                x.id_indikator='" . $rowlevelIKSK['id'] . "' and x.tahun=(select tahun
                                                from master_pk where id='1') and x.kode_satker=a.kode_satker limit 1),0) as capaian
                                                , ifnull((select x.capaian_outcome from tb_data_pk_awal x where x.deleted='0' and 
                                                x.id_indikator='" . $rowlevelIKSK['id'] . "' and x.tahun=(select tahun
                                                from master_pk where id='1') and x.kode_satker=a.kode_satker limit 1),0) as capaian_outcome
                                                , ifnull((select SUBSTRING_INDEX(nilai, '|', -1) from tb_data_pk_awal x where x.deleted='0' and 
                                                x.id_indikator='" . $rowlevelIKSK['id'] . "' and x.tahun=(select tahun
                                                from master_pk where id='1') and x.kode_satker=a.kode_satker limit 1),0) as nilai
                                                , (select count(zz.id) from tb_data_pk_tidak_cetak_awal zz where zz.id_indikator='" . $rowlevelIKSK['id'] . "'
                                            ...
                                            while ($rowceksatker = mysqli_fetch_assoc($resultceksatker)) {
                                                ...
                                                if ((float) $rowceksatker['terpilih'] == 0) {
                                                    ...
                                                    $sqlcekemonsatker = "select rumus,pagu,kode_emon,nmpaket,target, outcome, nilai, capaian, capaian_outcome from 
                                                        tb_data_pk_emon_awal where deleted='0' and tahun = (select tahun
                                                        from master_pk where id='1') and id_indikator = '" . $rowlevelIKSK['id'] . "'
                                                            ...
                                                            while ($rowcekemonsatker = mysqli_fetch_assoc($resultcekemonsatker)) {
                                                                ...
                                                                if ($rowlevelIKSK['hitungan_pk'] == '1') {
                                                                    ...
                                                                    [
                                                                        ...
                                                                        'nmpaket' => $rowcekemonsatker['nmpaket'],
                                                                        'pagu' => number_format($rowcekemonsatker['pagu'], $rowlevelIKSK['belakangkoma'], ",", "."),
                                                                        'target' => $targetnya_emon,
                                                                        'outcome' => $targetnya_emon,
                                                                        'capaian' => $capaiannya_emon,
                                                                        'capaian_outcome' => $capaiannya_emon,
                                                                        'kinerja' => number_format($kinerja_emon, 2, ",", "."),
                                                                        'kinerja_outcome' => number_format($kinerja_emon, 2, ",", "."),
                                                                    ];
                                                                }
                                                            }
                                                        }
    ...
    foreach ($myArray as $obj) {
        ...
        $sheet->getStyle('B' . $baris . ':F' . $baris)->getFont()->setBold(true);
        $sheet->getStyle('B' . $baris . ':F' . $baris)->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)->getStartColor()->setARGB($warna);
        $sheet->getStyle('B' . $baris . ':F' . $baris)->applyFromArray($styleRangeArray);
        ...
        foreach ($obj->balai as $balai) {
            ...
            $sheet->getStyle('B' . $baris . ':F' . $baris)->getFont()->getColor()->setARGB(\PhpOffice\PhpSpreadsheet\Style\Color::COLOR_BLUE);
            $sheet->getStyle('B' . $baris . ':F' . $baris)->getFont()->setBold(true);
            $sheet->getStyle('B' . $baris . ':F' . $baris)->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)->getStartColor()->setARGB("E0F20D");
            $sheet->getStyle('B' . $baris . ':F' . $baris)->applyFromArray($styleRangeArray);
            ...
            foreach ($balai->satker as $satker) {
                ...
                //////
                if ($obj->hitungan_pk == '1') {
                    $sheet->setCellValue('E' . $baris, replace_comma($satker->target));
                } else {
                    $sheet->setCellValue('E' . $baris, replace_comma($satker->nilai));
                }
                ...
                $sheet->setCellValue('F' . $baris, str_replace("&#37;", "%", str_replace("</sup>", "", str_replace("<sup>", "", $obj->namaoutcome))));
