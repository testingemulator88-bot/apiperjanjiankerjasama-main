...
if ($numcek > 0) {
    if (($Bearer <> '') && ($tempBearer[1] <> '')) {
        ...
        if ($kode_satkeremon_balai <> '' || $kode_satkeremon2_balai <> '') {
            ...
            x.kdsatker in (" . $kode_satkeremon_balai . ") or x.kdsatker in (" . $kode_satkeremon2_balai . ") group by x.kdgiat)";
        }
        ...
        if ($kode_satkeremon <> '' || $kode_satkeremon2 <> '') {
            $filter_query = $filter_query . "  and a.kdgiat in (select x.kdgiat from paket x where 
            x.kdsatker = '" . $kode_satkeremon . "' or x.kdsatker = '" . $kode_satkeremon2 . "' group by x.kdgiat)";
        }


        $myArray = array();
        $sql = "select a.idnya as id,a.kdgiat as kode
        ,a.nmgiat as nama from tgiat a where a.nmgiat<>'' " . $filter_query . "
        group by a.kdgiat,a.nmgiat order by a.kdgiat";
        //echo $sql;
        //die();
