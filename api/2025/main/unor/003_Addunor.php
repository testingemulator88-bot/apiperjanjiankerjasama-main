...
if ($numcek > 0) {
    ...
    if (($Bearer <> '') && ($tempBearer[1] <> '') && ($kdbalai <> '') && ($nama_kategori <> '') && ($alamat <> '')) {

        $sql = "insert into master_unor (id,kdbalai,kdbalai_old,nama_kategori,alamat,latitude,longitude
        ,createddate,createdby,deleted) values 
        (null,'" . $kdbalai . "','" . $kdbalai_old . "','" . $nama_kategori . "','" . $alamat . "','" . $latitude . "'
        ,'" . $longitude . "',SYSDATE(),'" . $createdby . "','0')";
        //echo $sql;
        //die;
