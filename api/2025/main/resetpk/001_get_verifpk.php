...
if ($numcek > 0) {
    if (($Bearer <> '') && ($tempBearer[1] <> '')) {
        $filterquery = "";
        $jenis_pk = fixup($_GET['jenis_pk']);
        if ($jenis_pk <> '') {
            $filterquery = $filterquery." and a.jenis_pk = '" . $jenis_pk . "'";
        }
        ...
        if ($kode_pelaksana == '') {
            $filterquery = $filterquery." and a.kode_pelaksana = '----'";
        }
        

        $myArray = array();
        $sql = "select a.*
        , (select b.nama from master_level_piu b where b.id = a.dari) as pengirim
        , (select b.nama from master_level_piu b where b.id = a.kepada) as penerima
