...
if ($numcek > 0) {
    if (($Bearer <> '') && ($tempBearer[1] <> '')) {
        ...
        $sqlindikator = "select * from tb_indikator_akhir where deleted='0' and id='" . $id . "' ";
        //echo $sqlindikator;
        //die;
        ...
        if ($numcekdatabaseline == 0) {
            $sqlinsert = "insert into tb_data_baseline_pk_balai_akhir (id, kode_tahun, id_indikator, id_balai, kode_balai
        ...
        for ($x = $tahun_awalcek; $x <= $tahun_akhircek; $x++) {
            ...
            from tb_data_pk_balai_akhir a where a.deleted='0' and a.tahun='" . $x . "' " . $filterquery . "";
            //echo $sqldata;
            $resultdatacek = mysqli_query($link, $sqldatacek);
            $numdatacek = mysqli_num_rows($resultdatacek);
            if ($numdatacek == 0) {
                $sqlinsert = "insert into tb_data_pk_balai_akhir (id, kode_tahun, id_indikator, tahun, id_balai, kode_balai, nama
                , kolom1, kolom2, kolom3, kolom4, kolom5, isian_kolom, nilai, rumus, createddate, createdby, deleted)
        ...
        if ($num > 0) {
            ...
            for ($x = $tahun_awal; $x <= $tahun_akhir; $x++) {
                ...
                if ($numdata > 0) {
                    if ($resultdata) {
                        $sqlrumus = "select rumuskomponen, kolomkomponen from tb_indikator_akhir where id = '" . $id . "'";
