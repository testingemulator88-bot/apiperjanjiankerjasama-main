        ...
        if(strstr($level, '1') || strstr($level, '2') || strstr($level, '5')){
            ...
            if(strstr($level, '1') || strstr($level, '2')){
                ...
                $sql_cek_user   = "SELECT b.kode_satker FROM master_satker as a 
                                ...
                                WHERE (b.level in (4,6,5,8,9) or a.level_piu = 21 or a.level_piu = 6 or a.level_piu = 31) and b.kode_satker = '". $kode_satker . "' GROUP BY b.kode_satker";
                // echo $sql_cek_user;
                // die;
            }
                                    ...
                                    if($kriteriaNum > 0){
                                        if($kriteriaResult){
                                            while($rowKriteria = mysqli_fetch_assoc($kriteriaResult)){
                                                ...
                                                if(strstr($level,'7')){
                                                    // $cek_lke        = "SELECT * FROM hasil_lke WHERE id_kriteria = '". $rowKriteria['id_kriteria'] ."' AND tahun = '". $tahun ."' AND id_user ='". $id_user ."' AND is_active = '1'";
                                                    $cek_lke        = "SELECT a.* FROM hasil_lke as a
                                                                        INNER JOIN tb_user as b on b.kode_satker = a.kode_satker and b.deleted='0' 
                                                                        WHERE a.id_kriteria = '". $rowKriteria['id_kriteria'] ."' AND a.tahun = '". $tahun ."' AND a.kode_satker ='". $id_user ."' AND a.level in (7) AND a.is_active = '1'
                                                                        GROUP BY a.id_hasil_lke
                                                                        ";
                                                }
                                                else{
                                                    // $cek_lke        = "SELECT * FROM hasil_lke WHERE id_kriteria = '". $rowKriteria['id_kriteria'] ."' AND tahun = '". $tahun ."' AND id_user ='". $id_user ."' AND kode_satker = '". $kode_satker ."' AND is_active = '1'";
                                                    $cek_lke        = "SELECT a.* FROM hasil_lke as a
                                                                        INNER JOIN tb_user as b on b.kode_satker = a.kode_satker and b.deleted='0' 
                                                                        WHERE a.id_kriteria = '". $rowKriteria['id_kriteria'] ."' AND a.tahun = '". $tahun ."' AND a.kode_satker ='". $id_user ."' AND a.level in (7) AND a.is_active = '1'
                                                                        GROUP BY a.id_hasil_lke
