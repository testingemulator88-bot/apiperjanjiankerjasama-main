...
if ($numcek > 0) {
    ...
    if (($Bearer <> '') && ($tempBearer[1] <> '') && ($id_indikator <> '')) {
        if ($kolom == 'nilai') {
            ...
            and kode_tahun = '" . $kode_tahun . "' and tahun = '" . $tahun . "' and kode_satker = '" . $kode_satker . "'";
            $resulttmp  = mysqli_query($link, $sqltemp);
            while ($rowtmp = mysqli_fetch_assoc($resulttmp)) {
                $ceknilai = $rowtmp[$kolom];
                $idnya = $rowtmp['id'];
                $tempnilai = explode("|", $ceknilai);
                $isian = "";
                for ($i = 0; $i < count($tempnilai); $i++) {
                    if ($i == 0) {
                        if ($i == (count($tempnilai) - 1)) {
                            $isian = $isian . replace_comma($isi);
                        } else {
                            $isian = $isian . $tempnilai[$i];
                        }
                    } else {
                        if ($i == (count($tempnilai) - 1)) {
                            $isian = $isian . "|" . replace_comma($isi);
