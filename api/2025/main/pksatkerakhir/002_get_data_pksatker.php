...
if ($numcek > 0) {
    if (($Bearer <> '') && ($tempBearer[1] <> '')) {
        ...
        $kodekegiatan = "";
        $sqlindikator = "  select kode from tb_indikator_akhir where id = 
        (select id_parent from tb_indikator_akhir where id = (select id_parent from tb_indikator_akhir where id = '" . $id . "'))";
        ...
        $sqlrumus = "select rumuskomponen from tb_indikator_akhir where id = '" . $id . "'";
        $resultrumus = mysqli_query($link, $sqlrumus);
        while ($rowrumus = mysqli_fetch_assoc($resultrumus)) {
            $rumus = $rowrumus['rumuskomponen'];
        }

        $sqlbelakangkoma = "select belakangkoma from tb_indikator_akhir where id = '" . $id . "'";
        $resultbelakangkoma = mysqli_query($link, $sqlbelakangkoma);
        while ($rowbelakangkoma = mysqli_fetch_assoc($resultbelakangkoma)) {
            $belakangkoma = $rowbelakangkoma['belakangkoma'];
        }
        $isinilai = "";
        $isinilaifinal = "";
                            ...
                            if ($resultdata) {
                                while ($rowdata = mysqli_fetch_assoc($resultdata)) {
                                    ...
                                    while ($rowemon = mysqli_fetch_assoc($resultemon)) {
                                        ...
                                        if ($belakangkoma == '3') {
                                            $tuliscapaian = replace_dot_koma_tiga($rowemon['capaian']);
                                            $tuliscapaian_outcome = replace_dot_koma_tiga($rowemon['capaian_outcome']);
                                        } else {
                                            $tuliscapaian = replace_dot_koma_tiga($rowemon['capaian']);
                                            $tuliscapaian_outcome = replace_dot_koma_tiga($rowemon['capaian_outcome']);
                                        }
                                        ...
                                        [
                                            ...
                                            'capaian' => $tuliscapaian,
                                            'capaian_outcome' => $tuliscapaian_outcome,
                                    ...
                                    if ($belakangkoma == '3') {
                                        $tuliscapaian = replace_dot_koma_tiga($rowdata['capaian']);
                                        $tuliscapaian_outcome = replace_dot_koma_tiga($rowdata['capaian_outcome']);
                                    } else {
                                        $tuliscapaian = replace_dot_koma($rowdata['capaian']);
                                        $tuliscapaian_outcome = replace_dot_koma($rowdata['capaian_outcome']);
                                    }
            ...
            for ($x = $tahun_awal; $x <= $tahun_akhir; $x++) {
                ...
                if ($numdata > 0) {
                    if ($resultdata) {
                        while ($rowdata = mysqli_fetch_assoc($resultdata)) {
                            $myArrayemon = array();
                            $sqlemon = "select a.id,a.tahun,a.kode_satker,a.kode_emon,a.outcome,a.target,a.nilai, a.rumus
