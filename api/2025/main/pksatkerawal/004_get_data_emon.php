...
if ($numcek > 0) {
    if (($Bearer <> '') && ($tempBearer[1] <> '')) {
        ...
        $sqlcekemonterpilih = "select GROUP_CONCAT(kode_emon) as cekemon from tb_data_pk_emon_awal where deleted='0' and id_pk='" . $id . "'";
        ...
        from paket_pk_awal a where a.nmpaket <>''
        " . $filterquery . " and YEAR(a.tanggaldata) = '" . $tahun . "'
        and a.tanggaldata = (select MAX(tanggaldata) from paket_pk_awal where YEAR(tanggaldata)= '" . $tahun . "')
        order by a.kode";
        //echo $sql;
        //die();
