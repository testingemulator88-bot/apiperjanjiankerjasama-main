...
if ($numcek > 0) {
    ...
    $singkatan = fixup($_POST['singkatan']);
    $createdby = fixup($_POST['createdby']);
    if (($Bearer <> '') && ($tempBearer[1] <> '') && ($nama <> '') && ($singkatan <> '')) {

        $sql = "insert into master_satuan (id,nama,singkatan
        ,createddate,createdby,deleted) values 
        (null,'" . $nama . "','" . $singkatan . "',SYSDATE(),'" . $createdby . "','0')";
        //echo $sql;
        //die;
