...
if ($numcek > 0) {
    ...
    if (($Bearer <> '') && ($tempBearer[1] <> '') && ($kdunor <> '') && ($tahun <> '')) {
        if ($mode == 'Tambah') {
            $sql = "insert into tb_indikator (id,tahun,id_parent,kdunor,urutlevel,level,kode,kode_unique,nama,satuan
            ...
            $sql = "update tb_indikator set tahun = '" . $tahun . "',kdunor = '" . $kdunor . "',nama = '" . $nama . "'
        ...
        $sql = "update tb_indikator set id_parent = null where id_parent ='0'";
        $result = mysqli_query($link, $sql);
        if ($result) {
            mysqli_close($link);
