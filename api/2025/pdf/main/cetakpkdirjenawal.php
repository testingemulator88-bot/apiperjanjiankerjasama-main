...
from tb_data_ttd_awal where kode_satker_pejabat = '" . $kodebalai . "' and level_pejabat='Dirjen'";
//echo $sqlsatkerttd;
//die;
...
from tb_data_ttd_awal where level_pejabat='Menteri'";
//echo $sqlbalaittd;
//die;
...
,ifnull((select sum(b.pg) from paket_pk_awal b where b.kdprogram=a.kdprogram),0) as pagu
from tprogram a
	...
	if ($resultPROGRAMLAMPIRAN2) {
		while ($rowPROGRAMLAMPIRAN2 = mysqli_fetch_assoc($resultPROGRAMLAMPIRAN2)) {
			...
			while ($rowKEGIATANLAMPIRAN2 = mysqli_fetch_assoc($resultKEGIATANLAMPIRAN2)) {
				...
				and a.id in (select b.id_parent from tb_indikator_awal b where b.deleted = '0' 
				and b.id in ((select c.id_indikator from tb_data_pk_awal c where b.deleted='0' and c.kode_satker in (" . $groupkode_satker . "))))
				order by a.id_parent ASC,a.urutlevel ASC, a.urut ASC ";
				...
				while ($rowSKLAMPIRAN2 = mysqli_fetch_assoc($resultSKLAMPIRAN2)) {
					...
					$sqlcektampilmaster = "select pelaksana from tb_indikator_awal where deleted='0' and id_parent = '" . $rowSKLAMPIRAN2['id'] . "'";
					$resultcektampilmaster = mysqli_query($link, $sqlcektampilmaster);
					$numcektampilmaster = mysqli_num_rows($resultcektampilmaster);


					$sqlcektampil = "select id from tb_data_pk_tidak_cetak_awal where deleted='0' and id_indikator in 
					(select id from tb_indikator_awal where deleted='0' and id_parent = '" . $rowSKLAMPIRAN2['id'] . "')
					...
					////
					$sqlcektampilINDIKATOR = "select pelaksana from tb_indikator_awal where deleted='0' and id_parent = '" . $rowSKLAMPIRAN2['id'] . "' ";
					...
					and a.pelaksana <> ''
					and a.id in (select b.id_indikator from tb_data_pk_awal b where b.deleted='0' and b.kode_satker in (" . $groupkode_satker . "))
					order by a.id_parent ASC,a.urutlevel ASC, a.urut ASC ";
					...
					while ($rowIKSKLAMPIRAN2 = mysqli_fetch_assoc($resultIKSKLAMPIRAN2)) {
						...
						$sqlcektampilIKSK = "select id from tb_data_pk_tidak_cetak_awal where deleted='0' and id_indikator = '" . $rowIKSKLAMPIRAN2['id'] . "'
						and kode_satker in (" . $groupkode_satker . ")";
						$resultcektampilIKSK = mysqli_query($link, $sqlcektampilIKSK);
						$numcektampilIKSK = mysqli_num_rows($resultcektampilIKSK);

						$sqlcektampilINDIKATOR = "select pelaksana from tb_indikator_awal where deleted='0' and id = '" . $rowIKSKLAMPIRAN2['id'] . "' ";
						//echo $sqlcektampilINDIKATOR."<br>";
						$resultcektampilINDIKATOR = mysqli_query($link, $sqlcektampilINDIKATOR);
						$numcektampilINDIKATOR = mysqli_num_rows($resultcektampilINDIKATOR);
						$jumlahcek = 0;
